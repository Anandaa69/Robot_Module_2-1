...
def capture_thread_func(manager: RMConnection, q: queue.Queue):
    ...
    while not stop_event.is_set():
        ...
        if fail >= 5:  # Reduced from 10 to 5
            ...
            try:
                ...
                pass
            fail = 0
        time.sleep(0.01)  # Increased from 0.005 to 0.01
...
def processing_thread_func(tracker: ObjectTracker, q: queue.Queue,
    ...
    while not stop_event.is_set():
        if not is_detecting_func():
            time.sleep(0.1); continue  # Increased from 0.05 to 0.1
        try:
            frame_to_process = q.get(timeout=0.5)  # Reduced from 1.0 to 0.5
            ...
            time.sleep(0.05)  # Increased from 0.02 to 0.05
...
def save_detected_objects_to_map(occupancy_map):
    """Save detected objects to map with position details"""
    ...
    if objects:
        ...
        # Save objects to map
        occupancy_map.save_objects_to_map(objects, CURRENT_POSITION, CURRENT_DIRECTION)
        
        # Also save to global detected_objects
        with object_lock:
            detected_objects.extend(objects)
        
        print(f"✅ Saved {len(objects)} objects to map at next position ({next_r}, {next_c})")
...
if __name__ == '__main__':
    ...
    if not manager.connected.is_set():
        print("⚠️ Camera not ready, retrying connection...")
        manager.connect()
        time.sleep(2.0)
    
    # Final check
    if not manager.connected.is_set():
        print("❌ Camera connection failed, continuing without object detection...")
        is_detecting_flag["v"] = False
    ...
    try:
        ...
        time.sleep(1.0)  # Wait for robot initialization
        ...
        for step in range(40):  # max_steps
            ...
            # --- INTEGRATED OBJECT DETECTION LOGIC ---
            if is_front_occupied and manager.connected.is_set():
                print("🔍 Wall detected within 60cm - Starting object detection...")
            ...
            elif is_front_occupied:
                print("📏 Wall detected but camera not ready - Skipping object detection")
            else:
                print("📏 No wall detected - No object detection needed")
