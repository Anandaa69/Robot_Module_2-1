...
class MazeMap:
    ...
    def set_wall(self, r, c, direction, has_wall):
        if 0 <= r < self.height and 0 <= c < self.width:
            # ===== LOGIC ที่แก้ไขใหม่ =====
            # ถ้าคำสั่งใหม่คือ "ไม่มีกำแพง" (has_wall=False)
            # แต่ในแผนที่เดิมบันทึกไว้แล้วว่า "มีกำแพง"
            # ให้เรา "คงข้อมูลเดิมไว้" และไม่ต้องทำอะไรต่อ
            # นี่คือการป้องกันไม่ให้กำแพงที่เคยเจอแล้วหายไป
            if not has_wall and self.grid[r][c].walls[direction]:
                return  # ออกจากฟังก์ชันทันที ไม่ต้องอัปเดต

            # ถ้าเงื่อนไขข้างบนไม่เป็นจริง ก็อัปเดตแผนที่ตามปกติ
            self.grid[r][c].walls[direction] = has_wall
            
            # อัปเดตกำแพงของช่องข้างเคียงให้ตรงกัน (เหมือนเดิม)
...
# ===== NEW FUNCTION =====
def perform_consistency_nudge(scanner, movement_controller, maze_map, current_pos, current_dir):
    """
    ตรวจสอบแผนที่จากช่องข้างเคียงที่เคยสำรวจแล้ว
    หากแผนที่ระบุว่ามีกำแพง แต่เซ็นเซอร์ปัจจุบันตรวจไม่พบ ให้ลองขยับเพื่อหา
    """
    print("--- Performing Consistency Nudge Check ---")
    r, c = current_pos
    
    # แปลงทิศทางของหุ่นยนต์เป็นเวกเตอร์และทิศทางสัมพัทธ์
    ...
    # ตรวจสอบกำแพงด้านซ้าย
    abs_dir_left = dir_map_rel_abs['left']
    abs_dir_left_name = dir_map_idx_name[abs_dir_left]
    nr_left, nc_left = r + dir_vectors[abs_dir_left][0], c + dir_vectors[abs_dir_left][1]
    
    # ตรวจสอบว่าช่องด้านซ้ายเคยไปมาแล้วหรือยัง
    if 0 <= nr_left < maze_map.height and 0 <= nc_left < maze_map.width and maze_map.grid[nr_left][nc_left].visited:
        # ถ้าแผนที่บอกว่า "ควรจะมีกำแพง"
        if maze_map.has_wall(r, c, abs_dir_left_name):
            current_scan, _ = scanner.get_wall_status()
            # แต่เซ็นเซอร์ปัจจุบัน "ตรวจไม่เจอ"
            if not current_scan['left']:
                print(f"   [Consistency] Map says wall on the LEFT, but sensor doesn't see it. Nudging LEFT to check.")
                # ขยับไปทางซ้ายเล็กน้อยเพื่อพยายามสแกนใหม่
                movement_controller.nudge_robot(y_speed=-0.25, duration=0.2)
                time.sleep(0.2)

    # ตรวจสอบกำแพงด้านขวา
    abs_dir_right = dir_map_rel_abs['right']
    abs_dir_right_name = dir_map_idx_name[abs_dir_right]
    nr_right, nc_right = r + dir_vectors[abs_dir_right][0], c + dir_vectors[abs_dir_right][1]

    # ตรวจสอบว่าช่องด้านขวาเคยไปมาแล้วหรือยัง
    if 0 <= nr_right < maze_map.height and 0 <= nc_right < maze_map.width and maze_map.grid[nr_right][nc_right].visited:
        ...
        if maze_map.has_wall(r, c, abs_dir_right_name):
            ...
            if not current_scan['right']:
                ...
                # ขยับไปทางขวาเล็กน้อยเพื่อพยายามสแกนใหม่
                movement_controller.nudge_robot(y_speed=0.25, duration=0.2)
                time.sleep(0.2)
...
def perform_centering_nudge(movement_controller, scanner, initial_wall_status):
    ...
    if has_left_wall_initial and not has_right_wall_initial:
        ...
        if status_after_nudge['right']:
            ...
            movement_controller.nudge_robot(y_speed=-(nudge_dist*0.95)/nudge_dur, duration=nudge_dur*0.95)
        return
...
def explore_with_ogm(scanner, movement_controller, attitude_handler, maze_map, visualizer, max_steps=40):
    ...
    with open("experiment_log.txt", "w") as log_file:
        ...
        for step in range(max_steps):
            ...
            # ===== UPDATED LOGIC =====
            # 1. ขยับเพื่อตรวจสอบความสอดคล้องของข้อมูลกับแผนที่เดิมก่อน
            perform_consistency_nudge(scanner, movement_controller, maze_map, CURRENT_POSITION, CURRENT_DIRECTION)

            # 2. ทำการสแกนและจัดกลางตามปกติ
            initial_wall_status, _ = scanner.get_wall_status()
            perform_centering_nudge(movement_controller, scanner, initial_wall_status)

            # 3. สแกนครั้งสุดท้ายเพื่อบันทึกลงแผนที่
