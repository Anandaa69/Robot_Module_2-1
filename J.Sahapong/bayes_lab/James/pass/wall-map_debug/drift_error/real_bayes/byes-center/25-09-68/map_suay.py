...
class RealTimeVisualizer:
    def __init__(self, grid_size, target_dest=None):
        ...
        self.colors = {"robot": "#0000FF", "target": "#FFD700", "path": "#FFFF00", "wall": "#000000", "wall_prob": "#000080"} # Navy for wall text
    ...
    def update_plot(self, occupancy_map, robot_pos, path=None):
        ...
        # 1. วาดพื้นหลังของ Node และค่าความน่าจะเป็นของ Node
        ...
        # 2. เพิ่มส่วนแสดงผลค่าความน่าจะเป็นของกำแพง (Wall Belief)
        for r in range(self.grid_size):
            for c in range(self.grid_size):
                cell = occupancy_map.grid[r][c]

                prob_n = cell.walls['N'].get_probability()
                if abs(prob_n - 0.5) > 0.01:
                    self.ax.text(c, r - 0.5, f"{prob_n:.2f}", ha="center", va="center", color=self.colors['wall_prob'], fontsize=6,
                                 bbox=dict(facecolor='white', alpha=0.7, boxstyle='round,pad=0.1', edgecolor='none'))

                prob_w = cell.walls['W'].get_probability()
                if abs(prob_w - 0.5) > 0.01:
                    self.ax.text(c - 0.5, r, f"{prob_w:.2f}", ha="center", va="center", color=self.colors['wall_prob'], fontsize=6, rotation=90,
                                 bbox=dict(facecolor='white', alpha=0.7, boxstyle='round,pad=0.1', edgecolor='none'))

        for c in range(self.grid_size):
            r_edge = self.grid_size - 1
            prob_s = occupancy_map.grid[r_edge][c].walls['S'].get_probability()
            if abs(prob_s - 0.5) > 0.01:
                self.ax.text(c, r_edge + 0.5, f"{prob_s:.2f}", ha="center", va="center", color=self.colors['wall_prob'], fontsize=6,
                             bbox=dict(facecolor='white', alpha=0.7, boxstyle='round,pad=0.1', edgecolor='none'))
        for r in range(self.grid_size):
            c_edge = self.grid_size - 1
            prob_e = occupancy_map.grid[r][c_edge].walls['E'].get_probability()
            if abs(prob_e - 0.5) > 0.01:
                self.ax.text(c_edge + 0.5, r, f"{prob_e:.2f}", ha="center", va="center", color=self.colors['wall_prob'], fontsize=6, rotation=90,
                             bbox=dict(facecolor='white', alpha=0.7, boxstyle='round,pad=0.1', edgecolor='none'))

        # 3. วาดเส้นกำแพงทึบ (หลังจากแสดงค่า prob)
        for r in range(self.grid_size):
            for c in range(self.grid_size):
                cell = occupancy_map.grid[r][c]
...
class MovementController:
    ...
    def move_precise_distance(self, distance_m, axis, attitude_handler):
        print(f"🔩 Moving precise distance: {distance_m * 100:.1f} cm")
        ...
        while time.time() - start_time < timeout:
            ...
            if relative_dist >= target_relative_dist:
                print("✅ Precise move complete.")
                break
...
def perform_centering_nudge(movement_controller, scanner, initial_readings, attitude_handler, occupancy_map, visualizer):
    ...
    if has_left_wall and not has_right_wall:
        print(f"   [Single Wall] Left wall detected. Recording wall to the '{dir_map_abs_char[left_dir_abs]}'...")
        occupancy_map.update_wall(r, c, dir_map_abs_char[left_dir_abs], True, 'ir')
        visualizer.update_plot(occupancy_map, CURRENT_POSITION)

        print("   -> Nudging RIGHT to find the opposite wall.")
        ...
        if readings_after['right']:
            print("   -> Found opposite wall. Recording opposite wall.")
            occupancy_map.update_wall(r, c, dir_map_abs_char[right_dir_abs], True, 'ir')
            visualizer.update_plot(occupancy_map, CURRENT_POSITION)
        else:
    ...
    if not has_left_wall and has_right_wall:
        print(f"   [Single Wall] Right wall detected. Recording wall to the '{dir_map_abs_char[right_dir_abs]}'...")
        occupancy_map.update_wall(r, c, dir_map_abs_char[right_dir_abs], True, 'ir')
        visualizer.update_plot(occupancy_map, CURRENT_POSITION)

        print("   -> Nudging LEFT to find the opposite wall.")
        movement_controller.slide_with_yaw_lock(y_speed=-y_speed, duration=nudge_dur, attitude_handler=attitude_handler)
        readings_after = scanner.get_sensor_readings(quiet=True)
    ...
    if not has_left_wall and not has_right_wall:
        ...
        if right_probe['right']:
            print("   -> Found a wall on the right. Recording wall.")
            occupancy_map.update_wall(r, c, dir_map_abs_char[right_dir_abs], True, 'ir')
            visualizer.update_plot(occupancy_map, CURRENT_POSITION)
            return
        ...
        if left_probe['left']:
            print("   -> Found a wall on the left. Recording wall.")
            occupancy_map.update_wall(r, c, dir_map_abs_char[left_dir_abs], True, 'ir')
            visualizer.update_plot(occupancy_map, CURRENT_POSITION)
            return

        print("   -> No walls found. Returning to original starting point.")
        movement_controller.slide_with_yaw_lock(y_speed=y_speed, duration=nudge_dur, attitude_handler=attitude_handler)
        return

    if has_left_wall and has_right_wall:
        print("   [Corridor] Walls detected on both sides. Recording both.")
        occupancy_map.update_wall(r, c, dir_map_abs_char[left_dir_abs], True, 'ir')
        occupancy_map.update_wall(r, c, dir_map_abs_char[right_dir_abs], True, 'ir')
        visualizer.update_plot(occupancy_map, CURRENT_POSITION)
...
def explore_with_ogm(scanner, movement_controller, attitude_handler, occupancy_map, visualizer, max_steps=40):
    ...
    for step in range(max_steps):
        ...
        perform_centering_nudge(movement_controller, scanner, initial_readings_for_nudge, attitude_handler, occupancy_map, visualizer)

        print("--- Performing Official Scan for Mapping (Front ToF Only) ---")
        front_dist_cm = scanner.get_front_tof_cm()
        is_front_occupied = front_dist_cm < scanner.tof_wall_threshold_cm
        print(f"[SCAN] Front (ToF): {front_dist_cm:.1f}cm -> {'OCCUPIED' if is_front_occupied else 'FREE'}")

        dir_map_abs_char = {0: 'N', 1: 'E', 2: 'S', 3: 'W'}
        front_dir_abs = CURRENT_DIRECTION
        
        occupancy_map.update_wall(r, c, dir_map_abs_char[front_dir_abs], is_front_occupied, 'tof')
