...
class GraphNode:
    def __init__(self, node_id, position):
        ...
        # Added from please.py logic to track out-of-bounds exits
...
class GraphMapper:
    ...
    def update_unexplored_exits_with_priority(self, node):
        ...
        for rel_dir in priority_order:
            ...
            # Boundary check logic integrated from your request
            tx, ty = target_pos
            if tx < self.min_x or tx > self.max_x or ty < self.min_y or ty > self.max_y:
                if abs_dir not in node.outOfBoundsExits:
                    node.outOfBoundsExits.append(abs_dir)
                print(f"🗺️  Direction {abs_dir} leads to {target_pos}, which is OUT OF BOUNDS. Skipping.")
                continue
...
# =============================================================================
# ===== NEW HELPER FUNCTION FOR SMOOTH MOVEMENT ===============================
# =============================================================================
def nudge_robot(chassis, speed_x, speed_y, duration):
    """
    ขยับหุ่นยนต์เล็กน้อยโดยใช้ drive_speed เพื่อความนุ่มนวล
    """
    print(f"🔩 Nudging robot: x_speed={speed_x}, y_speed={speed_y}, duration={duration}s")
    chassis.drive_speed(x=speed_x, y=speed_y, z=0, timeout=duration + 0.5)
    time.sleep(duration)
    chassis.drive_speed(x=0, y=0, z=0, timeout=0.5)
    time.sleep(0.2) # Wait for robot to settle

# =============================================================================
# ===== UPDATED SCANNING FUNCTION =============================================
# =============================================================================
def scan_current_node_absolute(gimbal, chassis, sensor, tof_handler, graph_mapper):
    ...
    if 0 < front_dist <= 19.0:
        print(f"⚠️ FRONT too close ({front_dist:.1f}cm). Nudging BACK.")
        nudge_robot(chassis, speed_x=-0.15, speed_y=0, duration=0.3)
    ...
    if 0 < left_dist < 15.0:
        print(f"⚠️ LEFT too close ({left_dist:.1f}cm). Nudging RIGHT.")
        nudge_robot(chassis, speed_x=0, speed_y=0.2, duration=0.25)

    # --- SCAN RIGHT ---
    gimbal.moveto(pitch=0, yaw=90, pitch_speed=speed, yaw_speed=speed).wait_for_completed()
    right_dist, right_wall = tof_handler.scan_direction(sensor, 'right'); scan_results['right'] = right_dist
    if 0 < right_dist < 15.0:
        print(f"⚠️ RIGHT too close ({right_dist:.1f}cm). Nudging LEFT.")
        nudge_robot(chassis, speed_x=0, speed_y=-0.2, duration=0.25)

    # --- BACK SCAN (for initial node only) & UPDATE GRAPH ---
    back_wall_scan = None
...
def generate_exploration_report(graph_mapper, movement_controller):
    ...
    filename = os.path.join(output_dir, "maze_data_2.json")
...
if __name__ == '__main__':
    ...
    # =========================================================================
    # ===== IMPORTANT: Define your map boundaries here ========================
    # =========================================================================
    # Example for 6x6 map, starting at bottom-right corner, facing North:
    # min_x is -5 (5 steps to the left)
    # max_x is 0 (0 steps to the right)
    # min_y is 0 (0 steps backward)
    # max_y is 5 (5 steps forward)
    ...
    movement_controller, attitude_handler = None, AttitudeHandler()
    try:
        ...
        if ep_robot:
            ...
            try:
                # Always try to generate a report, even on error
