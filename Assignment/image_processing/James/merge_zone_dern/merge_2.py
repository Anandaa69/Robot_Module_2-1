...
# =============================================================================
# ===== OBJECT DETECTION MODULE (from zone.py) ================================
# =============================================================================

class ObjectDetector:
    def __init__(self, template_paths):
        print("🖼️  Loading and processing template images for Object Detector...")
        self.templates = self._load_templates(template_paths)
        if not self.templates:
            print("❌ Could not load template files for Object Detector.")
        else:
            print("✅ Object Detector templates loaded successfully.")
        
        self.detection_thread = None
        self.detection_results = []

    # Night-robust color pre-processing
    def _apply_awb(self, bgr):
    ...
    def _retinex_msrcp(self, bgr, sigmas=(15, 80, 250), eps=1e-6):
    ...
    def _get_raw_detections(self, frame, target_shape, target_color, process_center_zone):
        enhanced = self._night_enhance_pipeline(frame)
        ...
        for cnt in contours:
            ...
            # Conditionally skip center zone objects
            if not process_center_zone and zone_label == "Center":
                print(f"   (Obj Detect) Skipping object in Center zone as per instruction (No wall ahead).")
                continue
            ...
            # Filter out uncertain objects in the center zone
            if shape == "Uncertain" and zone_label == "Center":
                 print(f"   (Obj Detect) Skipping UNCERTAIN object in Center zone.")
                 continue
            ...
            raw_detections.append({
                ...
                'color': found_color,
                'zone': zone_label,
                'is_target': (shape == target_shape and found_color == target_color),
                'is_uncertain': (shape == "Uncertain")
            })
        return raw_detections

    def _detection_worker(self, robot_camera, target_shape, target_color, wall_is_close):
        """This function runs in a separate thread."""
        print("\n📸 [Thread] Capturing frame for object detection...")
        try:
            frame = robot_camera.read_cv2_image(timeout=5)
            if frame is None:
                print("❌ [Thread] Failed to capture frame.")
                self.detection_results = []
                return

            # Crop the frame as in the original zone.py
            cropped_frame = frame[352:352+360, 14:14+1215]
            
            print(f"🧠 [Thread] Processing frame. Wall ahead: {wall_is_close}. Center zone processing: {'ENABLED' if wall_is_close else 'DISABLED'}.")
            
            self.detection_results = self._get_raw_detections(cropped_frame, target_shape, target_color, wall_is_close)
            
            if self.detection_results:
                print(f"✅ [Thread] Detection complete. Found {len(self.detection_results)} potential objects.")
                for res in self.detection_results: print(f"   -> Found: {res['color']} {res['shape']} in {res['zone']} zone.")
            else:
                print("🔵 [Thread] Detection complete. No objects found meeting the criteria.")

        except Exception as e:
            print(f"❌ [Thread] An error occurred during detection: {e}")
            self.detection_results = []

    def run_detection_thread(self, robot_camera, target_shape, target_color, wall_is_close):
        """Starts the detection process in a non-blocking thread."""
        if self.detection_thread and self.detection_thread.is_alive():
            print("⚠️ [Detector] Previous detection thread is still running. Waiting for it to finish.")
            self.detection_thread.join()

        self.detection_results = [] # Clear previous results
        self.detection_thread = threading.Thread(
            target=self._detection_worker,
            args=(robot_camera, target_shape, target_color, wall_is_close)
        )
        self.detection_thread.start()
        print("🚀 [Detector] Object detection thread started.")
...
# =============================================================================
SPEED_ROTATE = 480
LEFT_SHARP_SENSOR_ID = 1; LEFT_SHARP_SENSOR_PORT = 1; LEFT_TARGET_CM = 13.0
RIGHT_SHARP_SENSOR_ID = 2; RIGHT_SHARP_SENSOR_PORT = 1; RIGHT_TARGET_CM = 13.0
LEFT_IR_SENSOR_ID = 1; LEFT_IR_SENSOR_PORT = 2
RIGHT_IR_SENSOR_ID = 2; RIGHT_IR_SENSOR_PORT = 2
...
def get_target_choice():
    """Gets user input for the target object's shape and color."""
    VALID_SHAPES = ["Circle", "Square", "Rectangle_H", "Rectangle_V"]
    VALID_COLORS = ["Red", "Yellow", "Green", "Blue"]
    print("\n--- 🎯 Define Target Characteristics ---")
    while True:
        shape = input(f"Select Target Shape ({'/'.join(VALID_SHAPES)}): ").strip().title()
        if shape in VALID_SHAPES: break
        print("⚠️ Invalid shape, please try again.")
    while True:
        color = input(f"Select Target Color ({'/'.join(VALID_COLORS)}): ").strip().title()
...
class OGMCell:
    def __init__(self):
        ...
        self.objects = []  # <-- NEW: To store detected objects
...
class OccupancyGridMap:
    ...
    def add_object_to_node(self, r, c, object_data): # <-- NEW
        if 0 <= r < self.height and 0 <= c < self.width:
            self.grid[r][c].objects.append(object_data)
            print(f"맵에 추가됨 ({r},{c}): {object_data['color']} {object_data['shape']}")
...
class RealTimeVisualizer:
    ...
    def update_plot(self, occupancy_map, robot_pos, path=None):
        ...
        for r in range(self.grid_size):
            for c in range(self.grid_size):
                if occupancy_map.grid[r][c].objects:
                    obj_texts = []
                    for obj in occupancy_map.grid[r][c].objects:
                        color_char = obj['color'][0]
                        shape_char = obj['shape'][0] if obj['shape'] != "Uncertain" else ""
                        shape_str = f"{color_char}-{shape_char}"
                        if obj['is_uncertain']: shape_str += "?"
                        if obj['is_target']: shape_str = f"*{shape_str}*"
                        obj_texts.append(shape_str)
                    
                    display_text = "\n".join(obj_texts)
                    text_color = "red" if any(o['is_target'] for o in occupancy_map.grid[r][c].objects) else "purple"
                    self.ax.text(c, r, display_text, ha="center", va="center", color=text_color, fontsize=9, weight='bold')
        ...
        legend_elements = [ plt.Rectangle((0,0),1,1, facecolor='#8B0000', label=f'Node Occupied (P>{OCCUPANCY_THRESHOLD})'), plt.Rectangle((0,0),1,1, facecolor='#D3D3D3', label=f'Node Free (P<{FREE_THRESHOLD})'), plt.Line2D([0], [0], color=self.colors['wall'], lw=4, label='Wall Occupied'), plt.Rectangle((0,0),1,1, facecolor=self.colors['robot'], label='Robot'), plt.Rectangle((0,0),1,1, facecolor=self.colors['target'], label='Target'), plt.Line2D([0], [0], color='purple', marker='o', linestyle='None', label='*TARGET* / R-Sq / Y-C?') ]
        self.ax.legend(handles=legend_elements, loc='upper right', bbox_to_anchor=(1.6, 1.0))
...
class MovementController:
    ...
    def adjust_position_to_wall(self, sensor_adaptor, attitude_handler, side, sensor_config, target_distance_cm, direction_multiplier):
        ...
        while time.time() - start_time < MAX_EXEC_TIME:
            ...
            print(f"Adjusting {side}... Current: {current_dist:5.2f}cm, Target: {target_distance_cm:4.1f}cm", end='\r'); time.sleep(0.02)
...
class EnvironmentScanner:
    def __init__(self, sensor_adaptor, tof_sensor, gimbal, chassis):
        ...
        self.tof_wall_threshold_cm = 60.0; self.last_tof_distance_cm = float('inf')
    ...
    def get_sensor_readings(self):
        ...
        try:
            ...
            for side in ["Left", "Right"]:
                ...
                print(f"\n[SCAN] {side} Side: Sharp={'WALL' if is_sharp_detecting_wall else 'FREE'}, IR={'WALL' if is_ir_detecting_wall else 'FREE'}")
                if is_sharp_detecting_wall == is_ir_detecting_wall: is_wall = is_sharp_detecting_wall
                else:
                    print("    -> Ambiguity! Confirming with ToF..."); target_gimbal_yaw = -90 if side == "Left" else 90
                    try:
...
# =============================================================================
# ===== PATHFINDING & EXPLORATION LOGIC (MODIFIED) ============================
# =============================================================================
...
def execute_path(path, movement_controller, attitude_handler, scanner, visualizer, occupancy_map, path_name="Backtrack"):
    ...
    for i in range(len(path) - 1):
        ...
        if i + 1 < len(path):
            ...
            print(f"   -> [{path_name}] Performing side alignment at {CURRENT_POSITION}")
...
def perform_side_alignment_and_mapping(movement_controller, scanner, attitude_handler, occupancy_map, visualizer):
    ...
    visualizer.update_plot(occupancy_map, CURRENT_POSITION)
    if side_walls_present['left']: movement_controller.adjust_position_to_wall(scanner.sensor_adaptor, attitude_handler, "Left", scanner.side_sensors["Left"], LEFT_TARGET_CM, direction_multiplier=1)
    ...
    visualizer.update_plot(occupancy_map, CURRENT_POSITION)
    if side_walls_present['right']: movement_controller.adjust_position_to_wall(scanner.sensor_adaptor, attitude_handler, "Right", scanner.side_sensors["Right"], RIGHT_TARGET_CM, direction_multiplier=-1)
...
def explore_with_ogm(scanner, movement_controller, attitude_handler, occupancy_map, visualizer, object_detector, target_shape, target_color, robot_camera, max_steps=40):
    ...
    for step in range(max_steps):
        ...
        for target_dir in priority_dirs:
            ...
            if occupancy_map.is_path_clear(r, c, target_r, target_c) and (target_r, target_c) not in visited_cells:
                print(f"Path to {['N','E','S','W'][target_dir]} at ({target_r},{target_c}) seems clear. Preparing move.")
                movement_controller.rotate_to_direction(target_dir, attitude_handler)
                
                # <<< NEW INTEGRATED LOGIC >>>
                print("   -> Centering gimbal before ToF check...")
                scanner.gimbal.moveto(pitch=0, yaw=0, yaw_speed=SPEED_ROTATE).wait_for_completed(); time.sleep(0.1)
                
                is_front_wall_close_for_detection = scanner.get_front_tof_cm() < 90.0
                print(f"   -> Wall distance for object detection check: {scanner.last_tof_distance_cm:.1f} cm. Close enough: {is_front_wall_close_for_detection}")
                
                # Start object detection in a thread
                detection_thread = object_detector.run_detection_thread(robot_camera, target_shape, target_color, is_front_wall_close_for_detection)
                
                print("   -> Re-confirming path with ToF while detection runs...")
                is_blocked = scanner.last_tof_distance_cm < scanner.tof_wall_threshold_cm
                ...
                if not is_blocked:
                    ...
                    # After moving, wait for the detection thread to finish
                    print("   -> Waiting for object detection results...")
                    detection_thread.join(timeout=10) # Wait up to 10 seconds
                    if detection_thread.is_alive():
                        print("   -> ⚠️ Object detection timed out!")
                    
                    # Now we are at the new position, update it
                    CURRENT_POSITION = (target_r, target_c)
                    
                    # Add detected objects to the NEW current node
                    detected_objects = object_detector.detection_results
                    if detected_objects:
                        print(f"   -> Saving {len(detected_objects)} objects to map at {CURRENT_POSITION}")
                        for obj in detected_objects:
                            occupancy_map.add_object_to_node(CURRENT_POSITION[0], CURRENT_POSITION[1], obj)
                    
                    # Perform centering and update visualization
                    movement_controller.center_in_node_with_tof(scanner, attitude_handler)
                    visualizer.update_plot(occupancy_map, CURRENT_POSITION)
                    ...
                    print(f"   -> Confirmation failed. Path to {['N','E','S','W'][target_dir]} is blocked.")
                    # Wait for thread to finish even if we don't move, to avoid orphaned threads
        ...
        if not moved:
            ...
            if backtrack_path and len(backtrack_path) > 1:
                ...
                print("🎉 EXPLORATION COMPLETE! No reachable unvisited cells remain."); break
...
if __name__ == '__main__':
    ...
    try:
        # --- NEW: Get target object from user ---
        ...
        # --- NEW: Initialize Object Detector ---
        template_files = {
            "Circle": "./Assignment/image_processing/template/circle1.png", "Square": "./Assignment/image_processing/template/square.png",
            "Rectangle_H": "./Assignment/image_processing/template/rec_h.png", "Rectangle_V": "./Assignment/image_processing/template/rec_v.png",
        }
        object_detector = ObjectDetector(template_paths=template_files)

        scanner = EnvironmentScanner(ep_sensor_adaptor, ep_tof_sensor, ep_gimbal, ep_chassis)
        movement_controller = MovementController(ep_chassis)
        attitude_handler.start_monitoring(ep_chassis)
        
        explore_with_ogm(scanner, movement_controller, attitude_handler, occupancy_map, visualizer, object_detector, target_shape, target_color, ep_camera)
        ...
        if ep_robot:
            ...
            if attitude_handler.is_monitoring: attitude_handler.stop_monitoring(ep_chassis)
            if 'movement_controller' in locals() and movement_controller: movement_controller.cleanup()
            try: ep_robot.camera.stop_video_stream()
        ...
        for r in range(occupancy_map.height):
            for c in range(occupancy_map.width):
                ...
                cell_data = {
                    ...
                    "walls": {"north": cell.walls['N'].is_occupied(), "south": cell.walls['S'].is_occupied(), "east": cell.walls['E'].is_occupied(), "west": cell.walls['W'].is_occupied()},
                    "wall_probabilities": {"north": round(cell.walls['N'].get_probability(), 3), "south": round(cell.walls['S'].get_probability(), 3), "east": round(cell.walls['E'].get_probability(), 3), "west": round(cell.walls['W'].get_probability(), 3)},
                    "objects": cell.objects # <-- NEW: Add objects to JSON
                }
                final_map_data["nodes"].append(cell_data)

        with open("Mapping_Top.json", "w") as f: json.dump(final_map_data, f, indent=2)
        print("✅ Final Hybrid Belief Map (with objects) saved to Mapping_Top.json")
