...
class MovementController:
    ...
    def adjust_position_to_wall(self, sensor_adaptor, attitude_handler, side, sensor_config, target_distance_cm, direction_multiplier):
        ...
        # เพิ่ม Log ตรวจสอบค่าที่รับเข้ามา
        print(f"   -> Config: ID={sensor_config['id']}, Port={sensor_config['port']}, Target={target_distance_cm}cm")
        ...
        while time.time() - start_time < MAX_EXEC_TIME:
            # --- นี่คือส่วนที่แก้ไข ---
            # อ่าน ADC โดยตรงจาก sensor_adaptor ไม่ผ่าน scanner
            adc_val = sensor_adaptor.get_adc(id=sensor_config["id"], port=sensor_config["port"])
            ...
            # เพิ่ม Log ในลูปเพื่อดูการทำงาน
