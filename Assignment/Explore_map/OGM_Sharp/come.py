...
# --- Sharp Distance Sensor Configuration (from std.py) ---
...
SHARP_WALL_THRESHOLD_CM = 30.0  # ระยะสูงสุดที่จะถือว่าเจอผนัง
SHARP_STDEV_THRESHOLD = 0.8      # ค่าเบี่ยงเบนมาตรฐานสูงสุดที่ยอมรับได้ เพื่อกรองค่าที่แกว่ง

# --- Logical state for the grid map (from map_suay.py) ---
CURRENT_POSITION = (3, 0)  # (แถว, คอลัมน์)
...
class MovementController:
    ...
    def adjust_position_to_wall(self, scanner, attitude_handler, side, target_distance_cm, direction_multiplier):
        print(f"\n--- Adjusting {side} Side (Yaw locked at {CURRENT_TARGET_YAW:.2f}°) ---")
        TOLERANCE_CM, MAX_EXEC_TIME, KP_SLIDE, MAX_SLIDE_SPEED = 0.5, 8, 0.035, 0.15
        start_time = time.time()
        while time.time() - start_time < MAX_EXEC_TIME:
            current_dist, _ = scanner._get_stable_reading_cm(side, duration=0.1) # Fast reading
            if current_dist is None:
                print(f"\n[{side}] Lost wall during adjustment!")
                break
            dist_error = target_distance_cm - current_dist
            if abs(dist_error) <= TOLERANCE_CM:
                print(f"\n[{side}] Target distance reached!")
                break
            ...
            print(f"Adjusting {side}... DistErr: {dist_error:5.2f}cm", end='\r')
            time.sleep(0.02)
        else:
...
class EnvironmentScanner:
    ...
    def __init__(self, sensor_adaptor, tof_sensor, gimbal, chassis):
        ...
        self.side_sensors = {
            "Left":  {"id": LEFT_SENSOR_ADAPTOR_ID,  "port": LEFT_SENSOR_PORT},
            "Right": {"id": RIGHT_SENSOR_ADAPTOR_ID, "port": RIGHT_SENSOR_PORT}
        }
    ...
    def _get_stable_reading_cm(self, side, duration=0.7):
        ...
        while time.time() - start_time < duration:
            adc = self.sensor_adaptor.get_adc(id=sensor_info["id"], port=sensor_info["port"])
    ...
    def get_sensor_readings(self):
        """
        Performs a full scan: Front (ToF) and Sides (Sharp).
        ...
        for side in ["Left", "Right"]:
            ...
            print(f"[SCAN] {side} (Sharp) -> Avg Dist: {avg_dist:.2f} cm, Std Dev: {std_dev:.2f}")
            is_wall = (avg_dist < SHARP_WALL_THRESHOLD_CM and std_dev < SHARP_STDEV_THRESHOLD)
            readings[side.lower()] = is_wall
            print(f"    -> Result: {'WALL DETECTED' if is_wall else 'NO WALL'}")
...
def perform_side_alignment_and_mapping(movement_controller, scanner, attitude_handler, occupancy_map, visualizer):
    ...
    if side_walls_present['left']:
        movement_controller.adjust_position_to_wall(scanner, attitude_handler, "Left", LEFT_TARGET_CM, direction_multiplier=1)
    ...
    if side_walls_present['right']:
        movement_controller.adjust_position_to_wall(scanner, attitude_handler, "Right", RIGHT_TARGET_CM, direction_multiplier=-1)
