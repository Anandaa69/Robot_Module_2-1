...
# =============================================================================
# ===== MERGED CODE: ADVANCED NAVIGATION & MULTI-THREADED OBJECT DETECTION ====
# =============================================================================
# This script combines the robust navigation and mapping logic from 'debug_img_3-10.py'
# with the advanced, night-robust object detection from 'zone.py'.
#
# Key Features:
# - Multi-threaded object detection to prevent blocking robot movement.
# - ToF-based logic to conditionally process detection zones.
# - Detected objects are projected and saved onto the occupancy grid map.
# - Real-time visualizer shows both the map and the latest processed camera frame.
# - Core movement, alignment, and mapping algorithms from debug_img_3-10 are preserved.
# =============================================================================
...
# =============================================================================
# ===== CONFIGURATION & PARAMETERS (COMBINED) =================================
# =============================================================================

# --- Robot Movement & Navigation (from debug_img_3-10) ---
SPEED_ROTATE = 480
LEFT_SHARP_SENSOR_ID, LEFT_SHARP_SENSOR_PORT = 1, 1
LEFT_TARGET_CM = 13.0
RIGHT_SHARP_SENSOR_ID, RIGHT_SHARP_SENSOR_PORT = 2, 1
...
# --- Logical State (from debug_img_3-10) ---
...
# --- Occupancy Grid (from debug_img_3-10) ---
...
# --- NEW: Object Detection Configuration (from zone.py) ---
TARGET_SHAPE = "Circle"  # Will be redefined by user input
TARGET_COLOR = "Red"     # Will be redefined by user input
TOTAL_TARGET_OBJECTS_TO_FIND = 2 # Stop scanning after finding this many targets
TOF_WALL_AHEAD_THRESHOLD_CM = 80.0 # Threshold to decide if center zone is valid
g_found_target_count = 0 # Global counter for found targets

# =============================================================================
# ===== OBJECT DETECTION CODE (from zone.py) ==================================
# =============================================================================

# --- Night-robust color pre-processing ---
def apply_awb(bgr):
    # (Code from zone.py, no changes)
...
# --- ObjectTracker Class ---
class ObjectTracker:
    # (Code from zone.py, no changes to the class itself)
    ...
    def _get_angle(self, pt1, pt2, pt0):
        ...
        dot_product = dx1 * dx2 + dy1 * dy2; magnitude1 = math.sqrt(dx1 * dx1 + dy1 * dy1); magnitude2 = math.sqrt(dx2 * dx2 + dy2 * dy2)
        if magnitude1 * magnitude2 == 0: return 0
        return math.degrees(math.acos(dot_product / (magnitude1 * magnitude2)))
    def _get_raw_detections(self, frame):
        enhanced = night_enhance_pipeline(frame); enhanced = cv2.GaussianBlur(enhanced, (5, 5), 0)
        ...
        masks = {}; lower1, upper1, lower2, upper2 = color_ranges['Red']; masks['Red'] = cv2.inRange(normalized_hsv, lower1, upper1) | cv2.inRange(normalized_hsv, lower2, upper2)
        ...
        kernel = np.ones((5, 5), np.uint8); cleaned_mask = cv2.morphologyEx(cv2.morphologyEx(combined_mask, cv2.MORPH_OPEN, kernel, iterations=1), cv2.MORPH_CLOSE, kernel, iterations=2)
        ...
        for cnt in contours:
            ...
            shape = "Uncertain"; peri = cv2.arcLength(cnt, True); approx = cv2.approxPolyDP(cnt, 0.04 * peri, True); circularity = (4 * np.pi * area) / (peri * peri) if peri > 0 else 0
            if circularity > 0.84: shape = "Circle"
            elif len(approx) == 4:
                points = [tuple(p[0]) for p in approx]; angles = [self._get_angle(points[(i - 1) % 4], points[(i + 1) % 4], p) for i, p in enumerate(points)]
                if all(75 <= ang <= 105 for ang in angles):
                    _, (w, h), _ = cv2.minAreaRect(cnt); aspect_ratio = max(w, h) / min(w, h) if min(w, h) > 0 else 0
...
def scan_and_process_maze_wall(frame, tracker, target_shape, target_color):
    ...
    for det in detections:
        ...
        detailed_results.append({ "id": object_id_counter, "color": color, "shape": shape, "zone": zone_label, "is_target": is_target })
        cv2.rectangle(output_image, (x, y), (x + w, y + h), box_color, thickness)
        cv2.putText(output_image, str(object_id_counter), (x + 5, y + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.8, box_color, 2)
        object_id_counter += 1
    return output_image, detailed_results

# --- NEW: Detection Worker for Multithreading ---
def object_detection_worker(frame_queue, result_queue, tracker, target_shape, target_color):
    """This function runs in a separate thread, processing frames as they arrive."""
    print("🚀 Object detection thread started.")
    while True:
        try:
            # Wait for a new frame to be put in the queue
            frame, robot_pos_at_scan_time, robot_dir_at_scan_time, tof_dist_at_scan_time = frame_queue.get()
            
            if frame is None: continue # Skip if frame is invalid
                
            print(f"🧠 [Thread] Processing frame for position {robot_pos_at_scan_time}...")
            processed_image, detections = scan_and_process_maze_wall(frame, tracker, target_shape, target_color)
            
            # Put the results and the original context back into the result queue
            result_context = (robot_pos_at_scan_time, robot_dir_at_scan_time, tof_dist_at_scan_time)
            result_queue.put((processed_image, detections, result_context))
            
        except Exception as e:
            print(f"🔥 [Thread Error] An error occurred in the detection worker: {e}")

# =============================================================================
# ===== HELPER FUNCTIONS (from debug_img_3-10) ================================
# =============================================================================
...
def get_next_node(r, c, direction):
    """ NEW: Helper to calculate coordinates of the node in front of the robot. """
    if direction == 0: return (r - 1, c) # North
...
# =============================================================================
# ===== OCCUPANCY GRID MAP & VISUALIZATION (MODIFIED) =========================
# =============================================================================
class WallBelief:
    # (Code from debug_img_3-10, no changes)
...
class OGMCell:
    # --- MODIFIED: Added a list to store detected objects ---
    def __init__(self):
        self.log_odds_occupied = 0.0
        self.walls = {'N': None, 'E': None, 'S': None, 'W': None}
        self.detected_objects = [] # NEW
...
class OccupancyGridMap:
    # (Code from debug_img_3-10, no significant changes)
...
class RealTimeVisualizer:
    # --- MODIFIED: To display both map and processed image ---
    def __init__(self, grid_size, target_dest=None):
        self.grid_size = grid_size
        self.target_dest = target_dest
        plt.ion()
        # Create two subplots: one for the map, one for the image
        self.fig, (self.ax_map, self.ax_img) = plt.subplots(1, 2, figsize=(17, 8), gridspec_kw={'width_ratios': [1, 1.2]})
        self.colors = {"robot": "#0000FF", "target_dest": "#FFD700", "path": "#FFFF00", "wall": "#000000", "wall_prob": "#000080", "target_obj": "magenta", "uncertain_obj": "cyan"}

    def update_image(self, processed_image_bgr):
        """NEW: Updates the image subplot with the latest processed frame."""
        self.ax_img.clear()
    ...
    def update_plot(self, occupancy_map, robot_pos, path=None):
        ...
        for r in range(self.grid_size):
            for c in range(self.grid_size):
                ...
                # --- NEW: Draw markers for detected objects ---
                for obj in occupancy_map.grid[r][c].detected_objects:
                    if obj.get('is_target', False):
                        self.ax_map.plot(c, r, '*', color=self.colors['target_obj'], markersize=15, label="Detected Target")
                    elif obj.get('type') == 'uncertain':
                         self.ax_map.plot(c, r, 'o', color=self.colors['uncertain_obj'], markersize=8, mfc='none', label="Uncertain Object")
                    else: # Confident non-target
                        self.ax_map.plot(c, r, 's', color='black', markersize=8, mfc='none', label="Detected Object")

        # (Drawing walls, robot, path, etc. - code is the same, just changed 'ax' to 'ax_map')
        for r in range(self.grid_size):
            for c in range(self.grid_size):
        ...
        if self.target_dest:
            ...
            self.ax_map.add_patch(plt.Rectangle((c_t - 0.5, r_t - 0.5), 1, 1, facecolor=self.colors['target_dest'], edgecolor='k', lw=2, alpha=0.8))
        if path:
        ...
        if robot_pos:
            r_r, c_r = robot_pos
            self.ax_map.add_patch(plt.Rectangle((c_r - 0.5, r_r - 0.5), 1, 1, facecolor=self.colors['robot'], edgecolor='k', lw=2))
...
# =============================================================================
# ===== CORE ROBOT CONTROL CLASSES (from debug_img_3-10) ======================
# =============================================================================
# --- AttitudeHandler, PID, MovementController, EnvironmentScanner ---
# --- NO CHANGES are made to these core classes. They are copied verbatim ---
# --- to preserve the proven navigation and control logic.           ---
...
class MovementController:
    ...
    def center_in_node_with_tof(self, scanner, attitude_handler, target_cm=19, tol_cm=1.0, max_adjust_time=6.0):
        ...
        if tof_dist >= 50: print("[ToF] Distance >= 50cm, likely open space. Skipping."); return
        direction = 0
        ...
        if direction == 0: print(f"[ToF] Already centered within tolerance ({tof_dist:.2f} cm)."); return
...
# =============================================================================
# ===== PATHFINDING & EXPLORATION LOGIC (from debug_img_3-10) =================
# =============================================================================
# --- NO CHANGES are made to these core functions. ---
...
def find_nearest_unvisited_path(occupancy_map, start_pos, visited_cells):
    h, w = occupancy_map.height, occupancy_map.width; unvisited_cells_coords = []
    for r in range(h):
        for c in range(w):
    ...
    for target_pos in unvisited_cells_coords:
        ...
        if path:
            if shortest_path is None or len(path) < len(shortest_path): shortest_path = path
...
def execute_path(path, movement_controller, attitude_handler, scanner, visualizer, occupancy_map, path_name="Backtrack"):
    ...
    dir_vectors_map = {(-1, 0): 0, (0, 1): 1, (1, 0): 2, (0, -1): 3}; dir_map_abs_char = {0: 'N', 1: 'E', 2: 'S', 3: 'W'}
    for i in range(len(path) - 1):
        visualizer.update_plot(occupancy_map, path[i], path); current_r, current_c = path[i]
        if i + 1 < len(path):
...
def perform_side_alignment_and_mapping(movement_controller, scanner, attitude_handler, occupancy_map, visualizer):
    print("\n--- Stage: Wall Detection & Side Alignment ---"); r, c = CURRENT_POSITION
...
# =============================================================================
# ===== MAIN EXPLORATION LOGIC (MODIFIED for OBJECT DETECTION) ================
# =============================================================================
def explore_with_ogm(ep_robot, scanner, movement_controller, attitude_handler, occupancy_map, visualizer, frame_queue, result_queue, max_steps=40):
    global CURRENT_POSITION, CURRENT_DIRECTION, IMU_DRIFT_COMPENSATION_DEG, g_found_target_count
    visited_cells = set()
    ...
    for step in range(max_steps):
        ...
        # --- NEW: Check for and process detection results from the worker thread ---
        try:
            # Check the queue without blocking the main loop
            processed_image, detections, context = result_queue.get_nowait()
            robot_pos_at_scan_time, robot_dir_at_scan_time, tof_dist_at_scan_time = context

            print(f"📬 [Main] Received detection results for pos {robot_pos_at_scan_time}.")
            visualizer.update_image(processed_image) # Update the visualizer with the new image

            # Determine if the center zone is reliable
            is_center_zone_valid = (tof_dist_at_scan_time < TOF_WALL_AHEAD_THRESHOLD_CM)
            print(f"   -> ToF at scan time was {tof_dist_at_scan_time:.1f}cm. Center zone is {'VALID' if is_center_zone_valid else 'INVALID'}.")

            # Find the coordinates of the node in front of the robot at the time of the scan
            next_r, next_c = get_next_node(robot_pos_at_scan_time[0], robot_pos_at_scan_time[1], robot_dir_at_scan_time)

            if 0 <= next_r < occupancy_map.height and 0 <= next_c < occupancy_map.width:
                for det in detections:
                    should_save = False
                    # Logic to save the object
                    if det['shape'] != 'Uncertain': # Confident detection
                        if det['zone'] == 'Center' and is_center_zone_valid: should_save = True
                        elif det['zone'] in ['Left', 'Right']: should_save = True
                    elif det['shape'] == 'Uncertain' and det['zone'] in ['Left', 'Right']: # Uncertain, but only on sides
                        should_save = True
                    
                    if should_save:
                        print(f"   -> ✅ SAVING object {det['color']} {det['shape']} to map at ({next_r}, {next_c})")
                        obj_data = {'color': det['color'], 'shape': det['shape'], 'type': 'uncertain' if det['shape'] == 'Uncertain' else 'confident', 'is_target': det['is_target']}
                        occupancy_map.grid[next_r][next_c].detected_objects.append(obj_data)
                        if det['is_target']:
                            g_found_target_count += 1
                            print(f"   -> ⭐ TARGET FOUND! Total count: {g_found_target_count}/{TOTAL_TARGET_OBJECTS_TO_FIND}")

        except queue.Empty:
            # This is normal, means the worker thread is still processing or there's no new frame.
            pass
        ...
        # --- NEW: Perform Object Detection Scan ---
        if g_found_target_count < TOTAL_TARGET_OBJECTS_TO_FIND:
            print("--- Performing Scan for Object Detection ---")
            front_tof_cm = scanner.get_front_tof_cm()
            print(f"   -> Capturing frame, ToF is {front_tof_cm:.1f} cm...")
            captured_frame = ep_robot.camera.read_cv2_image(timeout=2)
            if captured_frame is not None:
                # Put the frame and current state into the queue for the worker thread
                frame_queue.put((captured_frame, CURRENT_POSITION, CURRENT_DIRECTION, front_tof_cm))
            else:
                print("   -> ❌ Failed to capture frame for object detection.")
        else:
            print("🎯 All targets found. Object detection scanning is now disabled.")

        # (The rest of the exploration logic is the same as before)
        is_front_occupied = scanner.last_tof_distance_cm < scanner.tof_wall_threshold_cm # Use last known value
        dir_map_abs_char = {0: 'N', 1: 'E', 2: 'S', 3: 'W'}; occupancy_map.update_wall(r, c, dir_map_abs_char[CURRENT_DIRECTION], is_front_occupied, 'tof')
        occupancy_map.update_node(r, c, False, 'tof'); visited_cells.add((r, c)); visualizer.update_plot(occupancy_map, CURRENT_POSITION)
        ...
        if not moved:
            print("No immediate unvisited path. Initiating backtrack..."); backtrack_path = find_nearest_unvisited_path(occupancy_map, CURRENT_POSITION, visited_cells)
            if backtrack_path and len(backtrack_path) > 1:
                execute_path(backtrack_path, movement_controller, attitude_handler, scanner, visualizer, occupancy_map); continue
...
if __name__ == '__main__':
    ...
    # --- NEW: Setup for Object Detection ---
    ...
    print(f"✅ Target defined: {TARGET_COLOR} {TARGET_SHAPE}. Finding {TOTAL_TARGET_OBJECTS_TO_FIND} of them.")
    ...
    frame_queue = queue.Queue(maxsize=1) # maxsize=1 to always process the latest frame
    result_queue = queue.Queue()
    
    # Create and start the detection worker thread
    detection_thread = threading.Thread(target=object_detection_worker, args=(frame_queue, result_queue, tracker, TARGET_SHAPE, TARGET_COLOR), daemon=True)
    ...
    try:
        ...
        explore_with_ogm(ep_robot, scanner, movement_controller, attitude_handler, occupancy_map, visualizer, frame_queue, result_queue)
        ...
        if ep_robot:
            ...
            try: ep_robot.camera.stop_video_stream() # Stop the camera stream
            except Exception as e: print(f"   (Note) Error stopping camera: {e}")
        ...
        # (Saving map data logic remains the same)
        plt.ioff(); plt.show()
