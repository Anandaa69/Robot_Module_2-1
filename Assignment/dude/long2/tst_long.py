...
def processing_thread_func(tracker: ObjectTracker, q: queue.Queue,
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            # --- Refactored aiming logic for detected target ---
            for d in detections:
                ...
                # Calculate offset from center (for aiming)
                target_center_x = x + w // 2
                target_center_y = y + h // 2
                frame_center_x = ROI_W // 2
                frame_center_y = ROI_H // 2
                x_offset = target_center_x - frame_center_x
                y_offset = target_center_y - frame_center_y

                # If target detected, use offset-based drive_speed for gimbal
                if is_target:
                    yaw_kp = 0.7
                    pitch_kp = 0.7
                    yaw_speed = int(np.clip(yaw_kp * x_offset, -MAX_YAW_SPEED, MAX_YAW_SPEED))
                    pitch_speed = int(np.clip(pitch_kp * y_offset, -MAX_PITCH_SPEED, MAX_PITCH_SPEED))
                    try:
                        manager.get_gimbal().drive_speed(yaw_speed=yaw_speed, pitch_speed=pitch_speed)
                        # Hold aim for 0.5s, then fire blaster 3 times
                        time.sleep(0.5)
                        ep_blaster = manager.get_blaster()
                        import robomaster.blaster as blaster
                        ep_blaster.fire(fire_type=blaster.WATER_FIRE, times=3)
                        time.sleep(0.5)
                    except Exception as e:
                        print(f"⚠️ Gimbal aiming/fire error: {e}")
