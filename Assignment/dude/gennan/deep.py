...
RESUME_MODE = False
DATA_FOLDER = r"D:\downsyndrome\year2_1\Robot_Module_2-1\Assignment\dude\superdude"
...
IMU_COMPENSATION_NODE_INTERVAL = 15
IMU_COMPENSATION_DEG_PER_INTERVAL = -1.0
...
def save_all_data(occupancy_map):
    try:
        ...
        print(f"✅ Final Hybrid Belief Map saved to {MAP_FILE}")
        ...
        print(f"✅ Detected objects saved to {OBJ_FILE}")
...
def capture_thread_func(manager: RMConnection, q: queue.Queue):
    ...
    while not stop_event.is_set():
        ...
        try:
            frame = cam.read_cv2_image(timeout=2.0)
...
def processing_thread_func(tracker: ObjectTracker, q: queue.Queue,
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            if time.time() - last_cleanup_time > 30.0:
                processing_count = 0
                last_cleanup_time = time.time()
                import gc
                gc.collect()
...
class RealTimeVisualizer:
    ...
    def update_plot(self, occupancy_map, robot_pos, path=None):
        ...
        display_objects = (self.update_counter % self.update_interval == 0)
        if display_objects:
    ...
    if len(path) >= 2:
        ...
        try:
            ...
            if check_for_targets():
                ...
                while is_tracking_mode and (time.time() - tracking_start_time) < 30:
