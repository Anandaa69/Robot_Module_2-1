...
PID_TIMEOUT = 7.5  # Timeout for PID tracking in seconds

# --- Centering Configuration ---
CENTERING_INTERVAL = 2  # Centering every N nodes (1 = every node, 2 = every 2 nodes)
...
def check_for_targets():
    ...
    if targets_found and not is_tracking_mode:
        ...
        # Sort targets by area (largest first) for priority
        def get_area(obj):
        ...
        targets_found.sort(key=get_area, reverse=True)
        print(f"🎯 Targets sorted by size: {[f\"ID:{t['id']} (area:{get_area(t)})\" for t in targets_found]}")
        ...
        if len(targets_found) > 0 and 'id' in targets_found[0]:
            ...
            shots_fired = 0
            print(f"🎯 Starting with largest target ID: {current_target_id}")
            return True
        else:
        ...
        if locked:
            if shots_fired < FIRE_SHOTS_COUNT:
                ...
                if targets_fired_count >= total_targets_count:
                    ...
                    # Still have targets to fire at, switch to next target
                    print(f"🎯 Still have {total_targets_count - targets_fired_count} targets remaining. Switching to next target...")
                    ...
                    if remaining_targets:
                        ...
                        shots_fired = 0
                        print(f"🎯 Switching to next target ID: {current_target_id}")
...
def execute_path(path, movement_controller, attitude_handler, scanner, visualizer, occupancy_map, path_name="Backtrack"):
    ...
    for i in range(len(path) - 2):  # หยุดที่โหนดก่อนโหนดที่ยังไม่สำรวจ
        ...
        if i + 1 < len(path):
            ...
            # Centering based on CENTERING_INTERVAL
            if (i + 1) % CENTERING_INTERVAL == 0:
                print(f"   -> [{path_name}] Centering at node {i+1} (every {CENTERING_INTERVAL} nodes)")
                movement_controller.center_in_node_with_tof(scanner, attitude_handler)
            else:
                print(f"   -> [{path_name}] Skipping centering at node {i+1} (centering every {CENTERING_INTERVAL} nodes)")
    ...
    if len(path) >= 2:
        ...
        try:
            ...
            if check_for_targets():
                ...
                while is_tracking_mode and (time.time() - tracking_start_time) < PID_TIMEOUT:  # Use global PID_TIMEOUT
        ...
        # Always center at the final destination
        print(f"   -> [{path_name}] Centering at final destination (always center at target)")
        movement_controller.center_in_node_with_tof(scanner, attitude_handler)
        ...
        for target_dir in priority_dirs:
            ...
            if occupancy_map.is_path_clear(r, c, target_r, target_c) and (target_r, target_c) not in visited_cells:
                ...
                if occupancy_map.is_path_clear(r, c, target_r, target_c):
                    ...
                    # Centering based on CENTERING_INTERVAL
                    if step % CENTERING_INTERVAL == 0:
                        print(f"   -> Centering at step {step+1} (every {CENTERING_INTERVAL} steps)")
                        movement_controller.center_in_node_with_tof(scanner, attitude_handler)
                    else:
                        print(f"   -> Skipping centering at step {step+1} (centering every {CENTERING_INTERVAL} steps)")
