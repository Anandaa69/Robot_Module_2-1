...
def explore_with_ogm(scanner, movement_controller, attitude_handler, occupancy_map, visualizer, max_steps=40):
    ...
    for step in range(max_steps):
        ...
        # ปรับปรุงกลยุทธ์การสำรวจ - สำรวจทุกทิศทางก่อนตัดสินใจ backtrack
        priority_dirs = [(CURRENT_DIRECTION + 1) % 4, CURRENT_DIRECTION, (CURRENT_DIRECTION - 1 + 4) % 4, (CURRENT_DIRECTION + 2) % 4]
        moved = False
        dir_vectors = [(-1, 0), (0, 1), (1, 0), (0, -1)]
        exploration_attempts = 0
        max_exploration_attempts = 2  # ลองสำรวจสูงสุด 2 รอบ
        
        while not moved and exploration_attempts < max_exploration_attempts:
            exploration_attempts += 1
            print(f"   -> Exploration attempt {exploration_attempts}/{max_exploration_attempts}")
        ...
        if not moved:
            ...
            # ตรวจสอบว่าโหนดปัจจุบันเป็นทางตันหรือไม่ - ปรับปรุงให้มีการลองใหม่
            print(f"🔍 Analyzing if current position {CURRENT_POSITION} is a dead end...")
            dead_end_retry_count = 0
            is_dead = False
            
            # ลองตรวจสอบ dead end หลายครั้งก่อนตัดสินใจ
            while dead_end_retry_count < 3:
                is_dead = is_dead_end(occupancy_map, CURRENT_POSITION, visited_cells, retry_count=dead_end_retry_count)
                if not is_dead:
                    print(f"⚠️ Position {CURRENT_POSITION} may still have accessible paths (retry {dead_end_retry_count + 1}).")
                    break
                else:
                    dead_end_retry_count += 1
                    if dead_end_retry_count < 3:
                        print(f"🔄 Dead end detected, retrying analysis ({dead_end_retry_count}/3)...")
                        time.sleep(0.5)  # รอสักครู่ก่อนลองใหม่
            
            if is_dead:
                print(f"🚫 Dead end confirmed at {CURRENT_POSITION} after {dead_end_retry_count} retries. Marking as fully explored.")
                # ทำเครื่องหมายว่าโหนดนี้เป็นทางตัน
                mark_cell_as_dead_end(occupancy_map, CURRENT_POSITION)
            else:
                print(f"⚠️ Position {CURRENT_POSITION} may still have accessible paths after retry.")
            ...
            if backtrack_path and len(backtrack_path) > 1:
                ...
                # ลองตรวจสอบอีกครั้งก่อนจบการสำรวจ
                print("🔍 No immediate backtrack path found. Performing final comprehensive search...")
                final_backtrack_path = find_nearest_unvisited_path(occupancy_map, CURRENT_POSITION, visited_cells)
                if final_backtrack_path and len(final_backtrack_path) > 1:
                    print(f"🎯 Found final backtrack target: {final_backtrack_path[-1]}")
                    execute_path(final_backtrack_path, movement_controller, attitude_handler, scanner, visualizer, occupancy_map)
                    print("Final backtrack complete. Resuming exploration.")
                    continue
                else:
                    print("🎉 EXPLORATION COMPLETE! No reachable unvisited cells remain.")
                    break
