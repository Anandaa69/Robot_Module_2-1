...
# # ✅ เปลี่ยนเป็นฟังก์ชันใหม่นี้
# def create_pink_mask(img_bgr):
#     hsv = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2HSV)  # แปลงครั้งเดียว BGR → HSV
#     mask = cv2.inRange(hsv, np.array([121, 27, 95]), np.array([157, 78, 187]))
#     mask = cv2.medianBlur(mask, 1)
#     return cv2.morphologyEx(mask, cv2.MORPH_CLOSE, np.ones((11, 11), np.uint8))
...
def processing_thread_func(templates_masked, params, use_pid=True):
    ...
    while not stop_event.is_set():
        ...
        # masked = cv2.bitwise_and(gray, gray, mask=create_pink_mask(rgb))
        # ✅ เปลี่ยนเป็นส่วนนี้
        proc_bgr = cv2.resize(frame, (int(params["PROCESSING_WIDTH"]), h_proc), interpolation=cv2.INTER_AREA)
        pink_mask = create_pink_mask(proc_bgr)  # ใช้ BGR โดยตรง
        gray = cv2.cvtColor(proc_bgr, cv2.COLOR_BGR2GRAY)  # แปลงเป็น GRAY ครั้งเดียว
        masked = cv2.bitwise_and(gray, gray, mask=pink_mask)
        ...
        if final_dets:
            ...
            if roi.size > 0:
                ...
                contours, _ = cv2.findContours(create_pink_mask(roi), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                if contours:
