    ...
    try:
        ...
        while True:
            ...
            if final_detections:
                ...
                # วาดกรอบสี่เหลี่ยมเดิมที่ได้จาก Template Matching
                ...
                # 1. สร้าง ROI (Region of Interest) จากกรอบที่ได้
                # ป้องกันไม่ให้ค่าพิกัดออกนอกขอบเขตของภาพ
                ...
                if roi_x2 > roi_x1 and roi_y2 > roi_y1:
                    ...
                    # 2. หา Mask และ Contours ภายใน ROI
                    ...
                    if contours:
                        # 3. หา Contour ที่ใหญ่ที่สุด และสร้าง Bounding Box ใหม่
                        main_contour = max(contours, key=cv2.contourArea)
                        x_real, y_real, w_real, h_real = cv2.boundingRect(main_contour)
                        
                        # 4. แปลงพิกัดกลับไปเป็นพิกัดของเฟรมหลัก
                        real_top_left = (roi_x1 + x_real, roi_y1 + y_real)
                        real_bottom_right = (roi_x1 + x_real + w_real, roi_y1 + y_real + h_real)

                        # 5. วาด Bounding Box ใหม่ (สีเหลือง) เพื่อเปรียบเทียบ
                        cv2.rectangle(frame, real_top_left, real_bottom_right, (0, 255, 255), 2) # สีเหลือง, เส้นบางกว่า
                # =========================================================================
