...
# ---------------------------------------------------
# ส่วนที่ 2: การทำงานหลัก (Main Program) - แก้ไข PID Stabilization
# ---------------------------------------------------
...
def main():
    ...
    # ยืนยันการ recenter และรอให้เสร็จ
    print("Recentering gimbal...")
    ep_gimbal.recenter(pitch_speed=200, yaw_speed=200).wait_for_completed()
    time.sleep(1) # ให้เวลา Gimbal หยุดสนิท
    ...
    try:
        ...
        while True:
            ...
            if final_detections:
                ...
                # 1. Scale coordinates back to original resolution
                ...
                # วาดกรอบสี่เหลี่ยมเดิม (Template Bounding Box)
                ...
                # 2. หา Contour Bounding Box ภายใน ROI เพื่อให้ได้ขอบเขตที่แม่นยำขึ้น
                roi_x1 = max(0, tl_adjusted[0])
                roi_y1 = max(0, tl_adjusted[1])
                roi_x2 = min(frame.shape[1], br_adjusted[0])
                roi_y2 = min(frame.shape[0], br_adjusted[1])
                ...
                if roi_x2 > roi_x1 and roi_y2 > roi_y1:
                    ...
                    if contours:
                        ...
                        # วาด Bounding Box ใหม่ (สีเหลือง/ฟ้า)
            ...
            if target_found:
                ...
                # ตรวจสอบว่าเพิ่งได้เป้าหมายใหม่หรือไม่ (ถ้า prev_errs ถูกรีเซ็ตเป็น 0)
                if prev_err_x == 0 and prev_err_y == 0:
                    # หากเพิ่งตรวจพบเป้าหมายครั้งแรก หรือเพิ่ง Re-acquire: 
                    # ให้ใช้เฉพาะ P-term และข้าม D และ I เพื่อป้องกันการกระโดด
                    D_term_x = 0
                    D_term_y = 0
                    accumulate_err_x = 0 # ยืนยันว่า I ถูกรีเซ็ตเมื่อเริ่มติดตามใหม่
                    accumulate_err_y = 0
                else:
                    # กรณีที่กำลังติดตามอยู่: คำนวณ D และ I ตามปกติ
                ...
                # อัพเดตค่าเก่าสำหรับเฟรมถัดไป
                prev_err_x = err_x
                prev_err_y = err_y
                prev_time = current_time
                
            else:
                # เป้าหมายหายไป: สั่งให้ Gimbal หยุดทันที และรีเซ็ตค่า PID ทั้งหมด
