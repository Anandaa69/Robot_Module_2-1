# File: best_multithread_singlefile_with_HSV_Tuning.py
# Description: Added a separate window with HSV trackbars for live color tuning
#              and a window to display the black-and-white mask output.
...
# --- MODIFIED: Reads from global HSV variables ---
def create_color_mask(img_rgb):
    """Creates a mask based on the global HSV trackbar values."""
...
def prepare_templates(original_template_paths, processing_scale):
    ...
    processed_paths = []
    # This function is now more critical, as changing HSV means templates might need regeneration.
    # For simplicity, we assume templates are pre-processed correctly for a general pink.
    # The live HSV tuning will primarily affect the live frame detection.
    for i, file_path in enumerate(original_template_paths):
        ...
        # Note: Template caching uses the default pink mask. The live tuning affects the camera feed.
        masked = cv2.bitwise_and(gray, gray, mask=create_color_mask(rgb))
        cv2.imwrite(output_filename, masked)
        print(f"  Saved cached template to {output_filename}")
    return processed_paths

# --- ส่วนของ Multithreading ---
# MODIFIED: Added "mask_frame" to the output dictionary
latest_frame = None; processed_output = {"annotated_frame": None, "mask_frame": None, "speed_x": 0.0, "speed_y": 0.0}
...
def processing_thread_func(templates_masked, params, use_pid=True):
    ...
    while not stop_event.is_set():
        ...
        # --- MODIFIED: Use the dynamic mask function ---
        live_mask = create_color_mask(rgb)
        masked_gray = cv2.bitwise_and(gray, gray, mask=live_mask)
        
        dets = []; colors = [(0,255,0),(255,165,0),(0,0,255)]
        for i, tmpl in enumerate(templates_masked):
            for tl, br, conf in match_template_masked(masked_gray, tmpl, THRESH):
        ...
        if final_dets:
            ...
            # ... (rest of the detection and drawing logic is the same) ...
            roi = frame[max(0,tl_adj[1]):min(frame.shape[0],br_adj[1]), max(0,tl_adj[0]):min(frame.shape[1],br_adj[0])]
            if roi.size > 0:
                # Use the dynamic mask on the ROI as well
                roi_mask = create_color_mask(cv2.cvtColor(roi, cv2.COLOR_BGR2RGB))
        ...
        # --- Display Info on Frame (unchanged) ---
        ...
        cv2.putText(frame, f"Kalman Dist: {kalman_dist:.2f} cm", (10, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 255), 2)
        cv2.circle(frame, (int(center_x_orig), int(center_y_orig)), 5, (255,0,255), -1)

        with output_lock:
            # MODIFIED: Add the mask frame to the output
            processed_output={"annotated_frame":frame, "mask_frame": live_mask, "speed_x":speed_x, "speed_y":speed_y}
    print("Processing thread stopped.")

# --- Main Thread: Controller & UI ---
def main():
    global processed_output
    # ADDED: Make globals writable from this function
    ...
    # --- ADDED: Create Trackbar window ---
    cv2.namedWindow("HSV Trackbars")
    def nothing(x): pass
    cv2.createTrackbar("H_MIN", "HSV Trackbars", H_MIN, 179, nothing)
    cv2.createTrackbar("H_MAX", "HSV Trackbars", H_MAX, 179, nothing)
    cv2.createTrackbar("S_MIN", "HSV Trackbars", S_MIN, 255, nothing)
    cv2.createTrackbar("S_MAX", "HSV Trackbars", S_MAX, 255, nothing)
    cv2.createTrackbar("V_MIN", "HSV Trackbars", V_MIN, 255, nothing)
    cv2.createTrackbar("V_MAX", "HSV Trackbars", V_MAX, 255, nothing)
    ...
    try:
        while True:
            # --- ADDED: Read values from trackbars and update globals ---
            ...
            with output_lock:
                s_x, s_y = processed_output["speed_x"], processed_output["speed_y"]
                ann_frame = processed_output["annotated_frame"]
                mask_frame = processed_output["mask_frame"]
            
            ep_gimbal.drive_speed(pitch_speed=-s_y, yaw_speed=s_x)
            
            if ann_frame is not None:
                cv2.imshow("Robomaster Detection", ann_frame)
            
            # --- ADDED: Display the mask frame in a new window ---
            if mask_frame is not None:
                cv2.imshow("Mask View", mask_frame)
