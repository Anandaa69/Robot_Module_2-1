...
def processing_thread_func(templates_masked, params):
    ...
    # --- [ปรับปรุง] เพิ่มค่าจำกัดสำหรับ I-Term (Anti-Windup) ---
    INTEGRAL_LIMIT_Y = 200 # ค่าจำกัดสำหรับตัวสะสมแกน Y
    ...
    while not stop_event.is_set():
        ...
        if found:
            ...
            # --- [ปรับปรุง] Anti-Windup: จำกัดค่าที่สะสมใน I-Term ---
...
def main():
    ...
    # --- [แก้ไขหลัก] เพิ่มค่า P และ I ของ Pitch ให้สูงขึ้นมากเพื่อสู้แรงโน้มถ่วง ---
    params = {
        "PROCESSING_WIDTH": 640.0,
        "pid_yaw":   (-0.3, -0.01, -0.01), # (P, I, D) - จูนค่า Yaw เล็กน้อย
        "pid_pitch": (-0.32, -0.6, -0.08),  # (P, I, D) - เพิ่ม P และ I สำหรับ Pitch อย่างมีนัยสำคัญ
