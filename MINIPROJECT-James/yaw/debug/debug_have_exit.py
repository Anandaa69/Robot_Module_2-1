...
class GraphNode:
    def __init__(self, node_id, position):
        ...
        self.exitsFound = {}  # เก็บ { 'north': True, 'south': False, ... }
# ===== Graph Mapper =====
class GraphMapper:
    def __init__(self):
        self.nodes = {}
...
def scan_current_node_absolute(gimbal, chassis, sensor, tof_handler, graph_mapper, movement_controller):
    ...
    if left_distance >= 300.0:  # 3 เมตรขึ้นไป = exit
        ...
        exit_dir_abs = direction_map[graph_mapper.currentDirection]

        current_node.exitsFound[exit_dir_abs] = True
        current_node.exploredDirections.append(exit_dir_abs)
        if exit_dir_abs in current_node.unexploredExits:
            current_node.unexploredExits.remove(exit_dir_abs)

        # ย้อนกลับไปโหนดเดิม
        print("🔙 Reversing to previous node after exit detection...")
        movement_controller.reverse_to_previous_node()
        # หมายเหตุ: ตรงนี้ต้องมี movement_controller เป็น parameter ของฟังก์ชัน
