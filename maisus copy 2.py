...
def processing_thread_func(tracker: ObjectTracker, q: queue.Queue,
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            # Get previous frame objects for ID stability
            previous_objects = []
            try:
                with output_lock:
            ...
            # Function to check if two objects are the same based on position and size
            def is_same_object(obj1, obj2, threshold=50):
                if obj1["color"] != obj2["color"] or obj1["shape"] != obj2["shape"]:
                    return False
                ...
                return distance < threshold and size_diff < 0.3

            # Assign stable IDs to objects
            for d in detections:
    ...
    if target_box is not None:
        ...
        if locked:
            if shots_fired < FIRE_SHOTS_COUNT:
                ...
                # Check if there are more targets to fire at
                remaining_targets = []
                for det in dets:
                    if (det.get("is_target", False) and 
                        det.get("id") not in fired_targets):
                ...
                if remaining_targets:
                    print(f"🎯 Found {len(remaining_targets)} remaining targets: {remaining_targets}")
                    # Switch to next target with smooth transition
                    print(f"🎯 Switching to next target ID: {remaining_targets[0]}")
                    # Stop gimbal movement first for smooth transition
                    try:
                        gimbal.drive_speed(pitch_speed=0, yaw_speed=0)
                        time.sleep(0.1)
                    except Exception as e:
                        print(f"⚠️ Error stopping gimbal during switch: {e}")
                    
                    current_target_id = remaining_targets[0]
                    shots_fired = 0
                    target_missing_frames = 0
                    print(f"🎯 Switched to target ID: {current_target_id}")
                    # Continue tracking without stopping
                else:
                    print("🎯 All targets completed! Returning gimbal to front...")
