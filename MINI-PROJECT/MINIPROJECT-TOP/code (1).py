...
if __name__ == "__main__":
    ...
    print("1. สแกนหาสีแดงในทุกทิศทาง")
    print("2. หา marker ในทิศทางที่เจอสีแดง")
    print("3. ทำการ slow yaw scan")
    print("=" * 70)
    ...
    try:
        ...
        print("\n🔴 === ขั้นตอนที่ 1: สแกนหาสีแดง ===")
        # สแกนหาสีแดงในทุกทิศทาง
        red_angles = enhanced_scanner.scan_for_red_color_first()
        
        if not red_angles:
            print("❌ ไม่เจอสีแดงในทิศทางใดเลย")
            print("🔚 หยุดการทำงาน")
            ep_robot.close()
            exit()
        ...
        print("\n🎯 === ขั้นตอนที่ 2: หา marker ในทิศทางที่เจอสีแดง ===")
        # สแกนหา marker ในทิศทางที่เจอสีแดง
        marker_results = enhanced_scanner.scan_markers_in_red_directions()
        
        if marker_results:
            print(f"✅ พบ marker ใน {len(marker_results)} ทิศทาง")
        else:
            print("⚠️ ไม่พบ marker ในทิศทางใดเลย")
        
        print("\n🔄 === ขั้นตอนที่ 3: ทำการ slow yaw scan ===")
        # ทำการ slow yaw scan ในทิศทางที่เจอสีแดง
        slow_scan_results = enhanced_scanner.slow_yaw_scan_in_red_directions()
        
        if slow_scan_results:
            print(f"✅ ทำการ slow yaw scan ใน {len(slow_scan_results)} ทิศทาง")
        else:
            print("⚠️ ไม่สามารถทำ slow yaw scan ได้")
        
        # แสดงผลลัพธ์รวม
        print("\n" + "="*70)
        print("📊 ผลลัพธ์การสแกนรวม")
        print("="*70)
        print(f"🔴 สีแดง: {len(red_angles)} ทิศทาง - {red_angles}")
        
        if marker_results:
            marker_count = len([k for k, v in marker_results.items() if v.get('marker_ids')])
            print(f"🎯 Marker: {marker_count} ทิศทาง")
        
        if slow_scan_results:
            print(f"🔄 Slow Yaw Scan: {len(slow_scan_results)} ทิศทาง")
