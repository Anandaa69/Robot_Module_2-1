...
def safe_gimbal_move(gimbal, pitch, yaw, speed=480, timeout=3.0, max_retries=2):
    """
    Safe gimbal movement with timeout and retry mechanism
    Returns: (success, error_message)
    """
    for attempt in range(max_retries + 1):
        try:
            print(f"🔄 Gimbal move attempt {attempt + 1}: pitch={pitch}°, yaw={yaw}° (timeout={timeout}s)")
            
            # Use a shorter timeout for gimbal movements
            result = gimbal.moveto(pitch=pitch, yaw=yaw, pitch_speed=speed, yaw_speed=speed)
            
            # Wait with timeout
            start_time = time.time()
            while not result.is_completed:
                if time.time() - start_time > timeout:
                    print(f"⚠️ Gimbal movement timeout after {timeout}s!")
                    break
                time.sleep(0.1)
            
            if result.is_completed:
                print(f"✅ Gimbal moved successfully to pitch={pitch}°, yaw={yaw}°")
                time.sleep(0.2)  # Brief stabilization
                return True, None
            else:
                print(f"⚠️ Gimbal movement not completed within timeout")
                
        except Exception as e:
            print(f"❌ Gimbal movement error (attempt {attempt + 1}): {e}")
            if attempt < max_retries:
                print(f"🔄 Retrying gimbal movement...")
                time.sleep(0.5)
            else:
                return False, str(e)
...
# ===== Graph Mapper =====
class GraphMapper:
    def __init__(self, min_x=0, min_y=0, max_x=-3, max_y=3):
        self.nodes = {}
    ...
    def update_unexplored_exits_absolute(self, node):
        ...
        # Dead end detection - include out-of-bounds as "blocked"
        blocked_count = sum(1 for blocked in node.walls.values() if blocked)
        out_of_bounds_count = len(node.outOfBoundsExits)
        total_blocked = blocked_count + out_of_bounds_count
        
        # A node is dead end if it has 3+ blocked directions AND no unexplored exits
        node.isDeadEnd = (total_blocked >= 3 and len(node.unexploredExits) == 0)
        
        if node.isDeadEnd:
            print(f"🚫 DEAD END CONFIRMED at {node.id}:")
            print(f"   🧱 Walls: {blocked_count}")
            print(f"   🌐 Out-of-bounds: {out_of_bounds_count}")
            print(f"   📊 Total blocked: {total_blocked}")
            print(f"   🔍 Unexplored: {len(node.unexploredExits)}")
    ...
    def find_next_exploration_direction_with_priority(self):
        ...
        # Check if this node should be marked as dead end due to boundaries
        total_directions = len(current_node.unexploredExits) + len(current_node.outOfBoundsExits)
        if total_directions == 0 and not current_node.isDeadEnd:
            print(f"🚫 Node {current_node.id} has no valid directions - marking as dead end")
            current_node.isDeadEnd = True
            if current_node.id in self.frontierQueue:
                self.frontierQueue.remove(current_node.id)
                print(f"🧹 Removed newly detected dead end {current_node.id} from frontier queue")
    ...
    def find_nearest_frontier(self):
        ...
        if not valid_frontiers:
            ...
            print(f"   🗺️ All nodes within boundary [{self.min_x},{self.max_x}] x [{self.min_y},{self.max_y}] explored")
            return None, None, None
...
def scan_current_node_absolute(gimbal, chassis, sensor, tof_handler, graph_mapper, marker_handler=None, ep_robot=None):
    ...
    # Reset gimbal to center before scanning
    print("🔄 Resetting gimbal to center before scanning...")
    success, error = safe_gimbal_move(gimbal, pitch=0, yaw=0, speed=480, timeout=5.0)
    if not success:
        print(f"⚠️ Failed to reset gimbal: {error}")
        print("🔄 Attempting gimbal recenter...")
        try:
            gimbal.recenter(pitch_speed=100, yaw_speed=100).wait_for_completed()
            time.sleep(1.0)
        except Exception as e:
            print(f"❌ Gimbal recenter failed: {e}")
    ...
    success, error = safe_gimbal_move(gimbal, pitch=0, yaw=0, speed=speed, timeout=3.0)
    if not success:
        print(f"❌ Failed to move gimbal to FRONT position: {error}")
        front_distance = 0.0
        front_wall = True
    ...
    success, error = safe_gimbal_move(gimbal, pitch=0, yaw=-90, speed=speed, timeout=3.0)
    if not success:
        print(f"❌ Failed to move gimbal to LEFT position: {error}")
        left_distance = 0.0
        left_wall = True
    ...
    success, error = safe_gimbal_move(gimbal, pitch=0, yaw=90, speed=speed, timeout=3.0)
    if not success:
        print(f"❌ Failed to move gimbal to RIGHT position: {error}")
        print("⚠️ Skipping RIGHT scan due to gimbal error")
        right_distance = 0.0
        right_wall = True  # Assume wall if can't scan
...
def explore_autonomously_with_absolute_directions(gimbal, chassis, sensor, tof_handler, graph_mapper, movement_controller, attitude_handler, marker_handler=None, ep_robot=None, max_nodes=20):
    ...
    while nodes_explored < max_nodes:
        ...
        print(f"🔍 Looking for next exploration direction...")
        current_node = graph_mapper.get_current_node()
        if current_node:
            print(f"   📊 Current node status:")
            print(f"      🔍 Unexplored exits: {current_node.unexploredExits}")
            print(f"      🌐 Out-of-bounds exits: {current_node.outOfBoundsExits}")
            print(f"      🚫 Is dead end: {current_node.isDeadEnd}")
        ...
        if next_direction:
            ...
            if can_move:
                ...
                # Re-check if node becomes dead end after removing this exit
                if current_node and len(current_node.unexploredExits) == 0:
                    blocked_count = sum(1 for blocked in current_node.walls.values() if blocked)
                    out_of_bounds_count = len(current_node.outOfBoundsExits)
                    if (blocked_count + out_of_bounds_count) >= 3:
                        print(f"🚫 Node {current_node.id} became dead end after removing blocked exit")
                        current_node.isDeadEnd = True
                        if current_node.id in graph_mapper.frontierQueue:
                            graph_mapper.frontierQueue.remove(current_node.id)
                continue
        ...
        if backtrack_attempts > 10:
            print(f"🚨 EMERGENCY BREAK: Too many backtrack attempts ({backtrack_attempts})")
            print(f"   Current position: {graph_mapper.currentPosition}")
            print(f"   Frontier queue: {graph_mapper.frontierQueue}")
            current_node = graph_mapper.get_current_node()
            if current_node:
                print(f"   Current node unexplored: {current_node.unexploredExits}")
                print(f"   Current node out-of-bounds: {current_node.outOfBoundsExits}")
            print("🛑 Stopping exploration to prevent hang")
            break
...
if __name__ == '__main__':
    ...
    # Initialize components with CONSERVATIVE boundaries for stability
    tof_handler = ToFSensorHandler()
    graph_mapper = GraphMapper(min_x=0, min_y=-1, max_x=3, max_y=1)  # 3x3 grid (stable size)
