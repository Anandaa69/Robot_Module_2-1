...
class OccupancyGridMap:
    ...
    def save_objects_to_map(self, objects, robot_position, robot_direction):
        """Save detected objects to the map with position details"""
        try:
            # Calculate next node position based on robot direction
            dir_vectors = [(-1, 0), (0, 1), (1, 0), (0, -1)]  # N, E, S, W
            next_r, next_c = robot_position[0] + dir_vectors[robot_direction][0], robot_position[1] + dir_vectors[robot_direction][1]
            
            # Check if next node is within map bounds
            if 0 <= next_r < self.height and 0 <= next_c < self.width:
                # Create object data for JSON
                object_data = {
                    "timestamp": time.time(),
                ...
                existing_data["detections"].append(object_data)
                
                with open(objects_file, "w", encoding="utf-8") as f:
                    json.dump(existing_data, f, indent=2)
                
                print(f"✅ Saved {len(objects)} objects to map at position ({next_r}, {next_c})")
                
        except Exception as e:
            print(f"❌ Error saving objects to map: {e}")

# Continue with the rest of the classes and functions...
# [The rest of the code would continue here with the remaining classes and functions from both files]
